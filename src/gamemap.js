...
export default class GameMap {
  ...
  draw(p5) {
    ...
    //debugger;
    if (this.lastBlockTime == 0 || p5.millis() - this.lastBlockTime > this.blockInterval) {
      //p5.text("time" + elapsed, 100, 100);
      //console.log("generate block")
      this.lastBlockTime = p5.millis();
      this.generateBlock(p5);
    }
    if (this.lastDownTime == 0 || p5.millis() - this.lastDownTime > this.downInterval) {
      //p5.text("time" + elapsed, 100, 100);
      //console.log("generate block")
  ...
  generateBlock(p5) {
    if (this.arrBlocks.length < 30) {
      let newBlock = new Block(p5, p5.random(['I', 'J', 'L', 'O', 'S', 'T', 'Z']));
      //debugger
